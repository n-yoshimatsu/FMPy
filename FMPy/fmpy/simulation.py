...
class Input(object):
    ...
    def __init__(self, fmu, modelDescription, signals, set_input_derivatives=False):
        ...
        if is_fmi1:
            ...
            setters['Clock']   = (fmu.fmi3SetClock,   fmi3.fmi3Clock)
...
def simulate_fmu(filename,
    ...
    if fmi_type is None:
        if fmu_instance is not None:
            ...
            elif model_description.coSimulation is not None:
                fmi_type = 'ModelExchange'
            elif model_description.scheduledExecution is not None:
                fmi_type = 'ScheduledExecution'

    if fmi_type not in ['ModelExchange', 'CoSimulation', 'ScheduledExecution']:
        raise Exception('fmi_type must be one of "ModelExchange" or "CoSimulation" or "ScheduledExecution"')
    ...
    elif fmi_type == 'ScheduledExecution':
        result = simulateSE(model_description, fmu, start_time, stop_time, solver, step_size, relative_tolerance, start_values, apply_default_start_values, input, output, output_interval, record_events, timeout, step_finished, validate)
...
def simulateSE(model_description, fmu, start_time, stop_time, solver_name, step_size, relative_tolerance, start_values, apply_default_start_values, input_signals, output, output_interval, record_events, timeout, step_finished, validate):
    ...
    vr_Clock1_Period =  1 * 1e-3 #  50 [ms]
    vr_Clock2_Period =  5 * 1e-3 # 100 [ms]
    vr_Clock3_Period = 10 * 1e-3 # 200 [ms]
    ...
    while time < stop_time:
        ...
        ############################
        # Clock event algorithm here.
        ############################

        print("P0:", time)
        input.apply(time)
        
        if ( time // vr_Clock1_Period ) %2 == 0 :
            fmu.activateModelPartition(1001, 1001, time)
            
        if ( time // vr_Clock2_Period ) %2 == 0 :
            fmu.activateModelPartition(1003, 1002, time)
            
        if ( time // vr_Clock3_Period ) %2 == 0 :
            fmu.activateModelPartition(1003, 1003, time)
